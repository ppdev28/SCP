...
function AppIcon({ app, size = 40 }: { app: AppService; size?: number }) {
  ...
  const fallback = app.category === 'Database' ? '◉' : app.category === 'Monitoring' ? '◌' : app.category === 'Security' ? '◇' : app.category === 'Media' ? '◈' : app.category === 'Development' ? '⌘' : app.category === 'Storage' ? '□' : app.category === 'Automation' ? '⚙' : '◆'
  const isImage = /^https?:\/\//i.test(app.icon) && !failed
  return <div style={{ width: size, height: size, borderRadius: Math.round(size * 0.22), background: T.bg, border: `1px solid ${T.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, overflow: 'hidden' }}>
    {isImage ? <img src={app.icon} alt="" width={Math.round(size * 0.58)} height={Math.round(size * 0.58)} onError={() => setFailed(true)} style={{ objectFit: 'contain', display: 'block' }} /> : <span style={{ fontSize: Math.round(size * 0.52), lineHeight: 1 }}>{app.icon || fallback}</span>}
  </div>
}
...
export default function ApplicationsView({ addToast }: { onDetail?: () => void; addToast: (m: string, t: any) => void }) {
  ...
  if (selected) {
    return <div style={{ padding: '22px 24px', maxWidth: 1100, width: '100%' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 20, marginBottom: 16, flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', gap: 14, alignItems: 'center' }}><AppIcon app={selected} size={52} /><div><h1 style={{ fontSize: 18, fontWeight: 700, color: T.text }}>{selected.name}</h1><div style={{ fontSize: 12, color: T.textDim, marginTop: 3 }}>{selected.description}</div><div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 7 }}><StatusBadge status={selected.status} /><span style={{ fontSize: 10, color: T.textDim }}>{selected.category} · v{selected.version}</span></div></div></div>
  ...
  return <div style={{ padding: '22px 24px', maxWidth: 1200, width: '100%' }}>
    ...
    <div style={{ display: 'flex', borderBottom: `1px solid ${T.border}`, marginBottom: 18 }}><button key="installed" onClick={() => setTab('installed')} style={{ padding: '8px 16px', background: 'none', border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 500, color: tab === 'installed' ? T.text : T.textDim, textTransform: 'capitalize', borderBottom: `2px solid ${tab === 'installed' ? T.accent : 'transparent'}`, marginBottom: -1, fontFamily: 'Inter,sans-serif' }}>Installed ({apps.length})</button><button key="available" onClick={() => setTab('available')} style={{ padding: '8px 16px', background: 'none', border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 500, color: tab === 'available' ? T.text : T.textDim, textTransform: 'capitalize', borderBottom: `2px solid ${tab === 'available' ? T.accent : 'transparent'}`, marginBottom: -1, fontFamily: 'Inter,sans-serif' }}>Available</button></div>
    <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}><Input value={search} onChange={setSearch} placeholder={`Search ${tab} apps…`} icon={<Search size={12} />} style={{ maxWidth: 300 }} /><div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>{categories.map(c => <button key={c} onClick={() => setCat(c)} style={{ padding: '4px 10px', background: cat === c ? T.accent : T.raised, border: `1px solid ${cat === c ? T.accent : T.border}`, borderRadius: 5, cursor: 'pointer', fontSize: 11, fontWeight: 500, color: cat === c ? '#fff' : T.textSub, fontFamily: 'Inter,sans-serif' }}>{c}</button>)}</div></div>
