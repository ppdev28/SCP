...
import { Plus, Trash2, Maximize2, RefreshCw, Copy, Check, ChevronDown } from 'lucide-react'
import { T } from '../lib/tokens'
import { Btn } from '../components/ui'
import { execTerminal } from '../lib/api'

interface Session { id: string; title: string; lines: string[]; input: string; cwd: string; running: boolean }

function mkSession(n: number): Session { return { id: String(n), title: `Terminal ${n}`, lines: ['Connected to homelab-server', 'Interactive shell — commands execute on the server.', ''], input: '', cwd: '/home/pepe', running: false } }
...
export default function TerminalView() {
  ...
  const submit = async () => {
    ...
    const command = sess.input.trim()
    update({ input: '', running: true, lines: [...sess.lines, `pepe@homelab-server:${sess.cwd.replace('/home/pepe', '~')}$ ${command}`] })
    try {
      const result = await execTerminal(command, sess.cwd)
      setSessions(prev => prev.map(s => s.id === active ? { ...s, cwd: result.cwd, running: false, lines: [...s.lines, ...(result.output ? result.output.replace(/\n$/, '').split('\n') : []), `pepe@homelab-server:${result.cwd.replace('/home/pepe', '~')}$ `] } : s))
    } catch (error) {
