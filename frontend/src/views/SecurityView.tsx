...
if (typeof document !== 'undefined' && !document.getElementById('scp-sidebar-icon-hover-fix')) {
  ...
  style.textContent = `
    ...
    aside > div:first-child > button:last-child:hover{opacity:1!important;pointer-events:auto!important}
  `
...
export default function SecurityView({ addToast }: { addToast: (m: string, t: any) => void }) {
  ...
  return (
    <div style={{ padding: '22px 24px', width: '100%', maxWidth: 1400 }}>
      ...
      {error && <div style={{ marginBottom: 14, padding: '10px 14px', borderRadius: 8, border: `1px solid ${T.red}35`, background: `${T.red}0c`, color: T.red, fontSize: 12 }}>{error}</div>}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
            <Card>
              ...
              {data.checks.map(c => (
                <div key={c.label} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 16px', borderBottom: `1px solid ${T.borderMuted}` }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 12, fontWeight: 500, color: T.text }}>{c.label}</div><div style={{ fontSize: 11, color: T.textDim, marginTop: 1 }}>{c.detail}</div></div>
                  {c.label === 'Auto security updates' && c.ok === false && <Btn size="xs" variant="danger" onClick={() => void fixUpdates()} disabled={fixing}>{fixing ? 'Fixing…' : 'Fix'}</Btn>}
                </div>
              ))}
            </Card>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <Card>
                ...
                {data.sessions.length === 0 ? <div style={{ padding: '18px 16px', fontSize: 12, color: T.textDim }}>No active login sessions detected.</div> : data.sessions.map(s => (
                  <div key={s.pid} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 16px', borderBottom: `1px solid ${T.borderMuted}` }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 12, fontWeight: 600, color: T.text, fontFamily: 'JetBrains Mono,monospace' }}>{s.user}@{s.from}</div><div style={{ fontSize: 11, color: T.textDim }}>Since {s.since} · {s.method} · PID {s.pid}</div></div>
              ...
              <Card>
                ...
                <div style={{ padding: '10px 16px', display: 'flex', gap: 10, alignItems: 'center' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 12px', background: `${data.firewallActive ? T.green : T.red}12`, border: `1px solid ${data.firewallActive ? T.green : T.red}28`, borderRadius: 7 }}><span style={{ width: 7, height: 7, borderRadius: '50%', background: data.firewallActive ? T.green : T.red }} /><span style={{ fontSize: 12, fontWeight: 600, color: data.firewallActive ? T.green : T.red }}>{data.firewallActive ? 'Active' : 'Inactive'}</span></div>
                  <span style={{ fontSize: 11, color: T.textDim }}>Rules read directly from UFW</span>
                </div>
                {showRules && <div style={{ padding: '0 16px 12px' }}>{data.firewallRules.length === 0 ? <div style={{ fontSize: 11, color: T.textDim, padding: '8px 0' }}>No parsed UFW rules.</div> : data.firewallRules.map((r, i) => <div key={`${r.number}-${i}`} style={{ display: 'grid', gridTemplateColumns: '32px 90px 58px 1fr', gap: 8, padding: '6px 0', borderBottom: `1px solid ${T.borderMuted}`, fontSize: 11, fontFamily: 'JetBrains Mono,monospace' }}><span style={{ color: T.textDim }}>{r.number || '—'}</span><span style={{ color: T.textSub }}>{r.to}</span><span style={{ color: r.action.includes('ALLOW') ? T.green : T.red, fontWeight: 600 }}>{r.action}</span><span style={{ color: T.textDim, overflow: 'hidden', textOverflow: 'ellipsis' }}>{r.from}</span></div>)}</div>}
              </Card>
            </div>
            ...
            <Card style={{ gridColumn: '1 / -1' }}>
              ...
              {data.events.length === 0 ? <div style={{ padding: '18px 16px', fontSize: 12, color: T.textDim }}>No SSH, UFW, or fail2ban events were found in the last 24 hours.</div> : data.events.map((e, i) => <div key={`${e.timestamp}-${i}`} style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '9px 16px', borderBottom: i < data.events.length - 1 ? `1px solid ${T.borderMuted}` : 'none' }}><span style={{ fontSize: 11, color: T.textDim, fontFamily: 'JetBrains Mono,monospace', flexShrink: 0, marginTop: 1, width: 64 }}>{formatEventTime(e.timestamp)}</span>{e.type === 'error' && <AlertTriangle size={13} color={T.red} style={{ flexShrink: 0, marginTop: 1 }} />}{e.type === 'warn' && <AlertTriangle size={13} color={T.yellow} style={{ flexShrink: 0, marginTop: 1 }} />}{e.type === 'success' && <CheckCircle2 size={13} color={T.green} style={{ flexShrink: 0, marginTop: 1 }} />}{e.type === 'info' && <Info size={13} color={T.accent} style={{ flexShrink: 0, marginTop: 1 }} />}<span style={{ fontSize: 11, color: T.textSub, fontFamily: 'JetBrains Mono,monospace', overflowWrap: 'anywhere' }}>{e.message}</span></div>)}
            </Card>
          </div>
        </>
      ) : null}

      <div style={{ marginTop: 12, display: 'flex', alignItems: 'center', gap: 6, color: T.textDim, fontSize: 10 }}><TerminalSquare size={11} /> Security data is read from UFW, OpenSSH, systemd, fail2ban, who, ss, and the system journal.</div>
    </div>
  )
}
