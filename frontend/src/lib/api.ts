import type { Container, ContainerStatus, HealthStatus, HostOverview } from './types'
...
type ApiActionResponse = { ok: boolean; id: string }
...
function mapStatus(state: string): ContainerStatus {
  switch (state.toLowerCase()) {
    ...
    case 'created': case 'exited': case 'dead': return 'stopped'
...
function formatPorts(ports: ApiPort[] = []) {
  const published = ports.filter((p) => p.publicPort !== undefined).map((p) => `${p.publicPort}:${p.privatePort}`)
...
function formatUptime(status: string, state: string) {
  if (state.toLowerCase() === 'running') return status.replace(/^Up\s+/i, '').replace(/\s+\(healthy\)$/i, '') || 'Running'
  return status || state
}
...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok) { const body = await response.json().catch(() => null) as { error?: string } | null; throw new Error(body?.error || `SCP API returned ${response.status}`) }
