...
func collectMonitoringHost() (MonitoringHost, error) {
	load, err := readLoad("/proc/loadavg")
	if err != nil {
	...
	cpu, err := readCPUUsage("/proc/stat", 100*time.Millisecond)
	if err != nil {
		return MonitoringHost{}, err
	}
	swapTotal, swapFree, err := readSwap("/proc/meminfo")
	if err != nil {
		return MonitoringHost{}, err
	}
	...
	return MonitoringHost{
		CPUUsagePercent:   cpu,
		Cores:             memoryCPUCount(),
		MemoryUsedBytes:   memory.UsedBytes,
		MemoryTotalBytes:  memory.TotalBytes,
		MemoryUsagePercent: memory.UsagePercent,
		SwapUsedBytes:     swapTotal - minUint64(swapTotal, swapFree),
...
func memoryCPUCount() int {
	return int(readCPUCount())
}

func readCPUCount() uint64 {
...
func readSwap(path string) (total uint64, free uint64, err error) {
...
func readNetworkTotals(path string) (uint64, uint64) {
...
func readDiskIO(path string) (uint64, uint64) {
	file, err := os.Open(path)
	if err != nil {
		return 0, 0
	}
	...
	for scanner.Scan() {
		...
		reads, errR := strconv.ParseUint(fields[5], 10, 64)
		writes, errW := strconv.ParseUint(fields[9], 10, 64)
		if errR != nil || errW != nil {
			continue
		}
		readBytes += reads * 512
		writeBytes += writes * 512
	}
	return readBytes, writeBytes
}

func collectMonitoringContainers(ctx context.Context, docker *client.Client) ([]MonitoringContainer, error) {
	result, err := docker.ContainerList(ctx, client.ContainerListOptions{All: false})
	if err != nil {
		return nil, fmt.Errorf("list running containers: %w", err)
	}
	containers := make([]MonitoringContainer, 0, len(result.Items))
	for _, item := range result.Items {
		statsReader, err := docker.ContainerStats(ctx, item.ID, client.ContainerStatsOptions{Stream: false})
		if err != nil {
			continue
		}
		var stats container.StatsResponse
		decodeErr := json.NewDecoder(statsReader).Decode(&stats)
		_ = statsReader.Close()
		if decodeErr != nil {
			continue
		}
		...
		containers = append(containers, MonitoringContainer{
			ID: item.ID,
			Name: name,
			CPUPercent: containerCPUPercent(stats),
			MemoryUsedBytes: memoryUsage(stats),
			MemoryLimitBytes: stats.MemoryStats.Limit,
			MemoryPercent: memoryPercent(stats),
			NetRXBytes: networkRX(stats),
			NetTXBytes: networkTX(stats),
			BlockReadBytes: blockIORead(stats),
			BlockWriteBytes: blockIOWrite(stats),
		})
	}
...
func memoryPercent(stats container.StatsResponse) float64 {
	...
	return float64(memoryUsage(stats)) * 100 / float64(limit)
}
...
func blockIORead(stats container.StatsResponse) uint64 {
	var total uint64
...
func blockIOWrite(stats container.StatsResponse) uint64 {
	var total uint64
	for _, entry := range stats.BlkioStats.IoServiceBytesRecursive {
